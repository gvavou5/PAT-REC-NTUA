...
addpath(genpath('C:\Users\dimst\Desktop\Pattern Recognition\Lab3\wekaToolbox\'));
addpath(genpath('C:\program files\Weka-3-6\'));
...
resact  = [];
    ...
    for neighbours = 1:4
        resact = [resact,[mean_NNR_set_3_act_pca(neighbours,1);mean_NNR_set_3_act_pca(neighbours,4:6)']];
    end

    resact = [resact,[mean_BAYES_set_3_act_pca(1);mean_BAYES_set_3_act_pca(4:6)']];
    resact = round(resact,5);
...
resval  = [];
    ...
    for neighbours = 1:4
        resval = [resval,[mean_NNR_set_3_val_pca(neighbours,1);mean_NNR_set_3_val_pca(neighbours,4:6)']];
    end

    resval = [resval,[mean_BAYES_set_3_val_pca(1);mean_BAYES_set_3_val_pca(4:6)']];
    resval = round(resval,5);
